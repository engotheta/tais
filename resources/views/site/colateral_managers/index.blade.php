...
@section('title')
{{label('lbl_colateral_managers')}}
@endsection

@section('breadcrumb')
<li class="breadcrumb-item active" aria-current="page">{{ label('lbl_colateral_managers') }}</li>
@endsection
...
<div class="about-page main-container container-fluid bg-white">
    <div class="col-12">
        ...
        <div class="row">
            <div class="col-lg-12 px-0 my-2 pr-lg-555 page-content has-shifting-underline">
                ...
                <div class="row mx-0">
					<div class="col-12 px-0 py-1">
						<h5 class="pb-2 position-relative page-title text-uppercase text-primary hover-text-primary"> 
							<span class="d-inline-block py-2 position-relative text-bold text-primary">
							  <span class="px-2222">{{ label('lbl_colateral_managers') }}</span>
							</span>
					...
					@if(count($colateralManagers) || isset($term))
					<div class="mt-1 col-12">
						 <div class="w-100 d-flex justify-content-end  text-center mt-3">
							 <form class="form-inline d-flex rounded-medium border-primaryy bg-dark-light overflow-hidden" 
								 action="{{url('search-colateral-managers')}}" style="width:360px">
								 @csrf
						 ...
						 <div class="box-shadow rounded-small border-none overflow-hidden  bg-white position-relative mb-4  mt-2 ">
							 ...
							 @foreach ($colateralManagers as $colateralManager)
								 <div data-name="{{$colateralManager->name}}" class="searchable-record">
									 <div href="#" class="row text-hover-primary cursor-pointerrr  hover-bg-secondary-faded border-top">
										 <div class="col-md-3 col-sm-4 col-6 pl-4 d-flex align-items-center py-2"> {{$colateralManager->name}}</div>
										 <div class="col-md-3 col-sm-4 col-6  px-2 d-flex align-items-center border-left py-2">{{$colateralManager->phone}} </div>
										 <div class="col-md-3 d-none d-md-flex  px-2  align-items-center border-left py-2">{{$colateralManager->email}} </div>
										 <div class="col-md-3 col-sm-4 d-none d-sm-flex align-items-center pr-4 border-left py-2">{{$colateralManager->address}}  </div>
									 </div>
								 </div>
							 @endforeach
							 @if(!count($colateralManagers))
								  
								 <a href="{{url('colateralManagers')}}" class="d-block text-center p-3">
