					...
					<div class="row align-items-center">
							<div class="col-md-7">
									...
									<ul class="breadcrumb">
											...
											<li class="breadcrumb-item"><a href="javascript:">Stakeholder Categories</a></li>
									</ul>
							</div>
							...
							<div class="col-md-5 text-right">
									...
									<a href="{{ URL::route('cms.stakeholder-categories.create') }}" data-toggle="modal" data-target="#newCategory" class="btn btn-primary">Create New Category</a>
							</div>
					</div>
			</div>
	</div>
	...
	<div class="main-body">
			<div class="page-wrapper">
				...
				<div id="grid-view">
					<div class="row">
						@foreach($categories as $key => $category)
							<a class="col-md-2 my-3" href="{{ url('/cms/stakeholder-categories/'.$category->slug)}}">
								<div class="ffolder small {{ ($category->active == 1)? 'gray': 'pink'}}">
										<span class="text-white">{{ number_format(count($category->stakeholders))}} files</span>
								</div>
				...
				<div class="row" id="list-view">
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header" > <h5>List Of Stakeholder Categories</h5></div>
							<div class="card-body">
								...
								@else
									...
									@foreach($categories as $key => $category)
										<tr>
											...
											<td>{{ number_format(count($category->stakeholders)) }}</td>
											<td>@if ($category->active ==1) Active @else in Active	@endif</td>
											<td>
												<a href="{{ URL::route('cms.stakeholder-categories.show', $category->slug) }}" class="label theme-bg text-white f-12">View</a>
												<a href="{{ URL::route('cms.stakeholder-categories.edit', $category->id) }}" data-toggle="modal" data-target="#OpenModelContainer{{ $key }}"  class="label theme-bg2 text-white f-12">Edit</a>
												{!! link_to_route('cms.stakeholder-categories.destroy', "Delete", array($category->id), array('data-method' => 'delete', 'data-confirm' => 'Are you Sure' ,'class' => 'label theme-b bg-danger text-white f-12')) !!}
											</td>
